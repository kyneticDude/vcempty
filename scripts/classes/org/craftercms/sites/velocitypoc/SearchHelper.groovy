...
class SearchHelper {
  ...
  static final String[] HIGHLIGHT_FIELDS = ["title_s", "postSection_o.item.sectionContent_html"]
  ...
  def search(userTerm, categories, start = DEFAULT_START, rows = DEFAULT_ROWS) {
    ...
    if (userTerm) {
      ...
      def userTermQuery = "(title_s:(${userTerm}) OR postSection_o.item.sectionContent_html:(${userTerm}))"
  ...
  def searchArticles(featured, categories, start = DEFAULT_START, rows = DEFAULT_ROWS, additionalCriteria = null) {
    def q = "${ARTICLE_CONTENT_TYPE_QUERY}"
